...
This project is a collection of helm charts, insipired by the [AI Software Templates](https://github.com/redhat-developer/rhdh-ai-template).
...
The AI helm charts exclusively uses tekton pipelines and tasks under [rhdh-ai-template-pipelines](https://github.com/redhat-developer/rhdh-ai-template-pipelines) repo. The only customized resource used for the helm chart case is:
...
- A `tag` should be created with the version of the release as the name. `ai-lab-helm-charts` follows the {major}.{minor}.{bugfix} format (e.g 0.1.0).
- Before proceeding, make sure that all the `version` fields within `Chart.yaml` have this tag as the value. For example in the case where the tag is `0.1.0`, the `version` should be `0.1.0`.
