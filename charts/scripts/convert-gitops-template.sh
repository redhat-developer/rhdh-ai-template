    ...
    for file in "$gitops_templates_dir"/*.yaml; do
        if [[ -f $file ]]; then
            ...
            if [[ $INCLUDE_FILES == *$relFile* ]]; then
                convert "$file"
                cp "$file" "$root_dir/$CHART_PATH/templates"
            else
                echo "Skip $file"
            fi
        fi
...
APP_DIR=$(realpath "$ROOT_DIR"/../apps/ai)
...
# Copy the gitops directory into temporary directory
cp -r "$APP_DIR" "$TEMP_DIR"

for env in "$ENVS_DIR"/*; do
    # reset the temporary directory
    rm -rf "$TEMP_DIR" && cp -r "$APP_DIR" "$TEMP_DIR"
